...
假设在不改变原始 SQL 语句的情况下，从 `写入数据库` 和 `数据库中读取` 两个环节对敏感数据进行处理，同时为了减少修改原有代码（在代码里通过注解的方式或者其它方式），希望能够在配置文件中去配置敏感字段。
...
根据此需求，参考官方文档中选择`Executor` 中 `update`方法和 处理结果集 `ResultSetHandler` 的 `handleResultSets` 方法作为拦截器的签名。
