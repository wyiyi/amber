...
2. 访问内置对象
   ...
   ① 访问请求对象：
    ...
    ```
   [Thymeleaf 文档](https://www.thymeleaf.org/doc/tutorials/3.1/usingthymeleaf.html#base-objects) 中 '$#ctx.request}' 可通过 "$#ctx.request}" 在上下文对象中获取 request，也可以通过简化形式 ${#request} 获得到 request。再通过 request 中的 requestURI 获得。
